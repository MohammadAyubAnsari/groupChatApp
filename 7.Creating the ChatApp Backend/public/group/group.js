...
window.addEventListener("DOMContentLoaded", async () => {
  const response = await axios.get(`http://localhost:3000/group/getGroup`, {
  ...
  const result = await axios.post(`http://localhost:3000/group/getAllGroups`, {
    groupsId: groupsId,
  });
    ...
    if (confirm(`Are you sure ?`)) {
      try {
        ...
        const response = await axios.get(
          `http://localhost:3000/group/exitGroup/${id}`,
          { headers: { Authorization: token } }
        );
...
otherGroups.addEventListener("click", async (e) => {
  if (e.target.classList.contains("join")) {
    ...
    const id = e.target.parentNode.id;
    const result = await axios.get(`http://localhost:3000/group/join/${id}`, {
      headers: { Authorization: token },
    });
    // console.log(result);
