  ...
  try {
    ...
    if (isStringValid(email)) {
      ...
      if (!checkUserIsAdmin.isAdmin) {
        return res
          .status(500)
          .json({ success: false, message: `Only admin can add users !` });
      }

      if (req.user.email == email) {
        return res
          .status(500)
          .json({ success: false, message: `Admin is already in group !` });
      }
      ...
      if (alreadyInGroup) {
        return res
          .status(500)
          .json({ success: false, message: `User is already in group !` });
      }
      ...
      res
        .status(200)
        .json({ success: true, message: "User successfully added !", data });
    }
  } catch (err) {
    console.log(err);
    res.status(500).json({ success: false, message: `Something went wrong !` });
...
const deleteUser = async (req, res, next) => {
  try {
    ...
    if (!checkUser) {
      return res
        .status(400)
        .json({ success: false, message: `You are no longer in group !` });
    }
    ...
    if (req.user.email == email) {
      if (allAdmins.length > 1) {
        ...
        return res
          .status(400)
          .json({ success: false, message: `Make another user as an Admin !` });
      }
    }
...
const removeAdmin = async (req, res, next) => {
  ...
  try {
    ...
    if (checkUserIsAdmin.isAdmin == false) {
      return res
        .status(500)
        .json({ success: false, message: `Only Admin have this permission !` });
    }
